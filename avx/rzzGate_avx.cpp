...
#pragma omp parallel for schedule(static)
    for (ull qq = 0; qq < numAmps; qq += SIMD)
    {
...

        {
            ...
            if (isFirstLayer)
            {
                alignas(64) double tmp_r[8], tmp_i[8];
                __m512d HHs = _mm512_set1_pd(H);
                _mm512_store_pd(tmp_r, _mm512_mul_pd(HHs, contracted_rzz[0]));
                _mm512_store_pd(tmp_i, _mm512_mul_pd(HHs, contracted_rzz[1]));
                for (int i = 0; i < SIMD; i++) {
                    qureg.stateVec[qq + i].real = tmp_r[i];
                    qureg.stateVec[qq + i].imag = tmp_i[i];
                }
            }
            else
            {
                ...
                alignas(64) double tmp_r[8], tmp_i[8];
                for (int i = 0; i < SIMD; i++) {
                    tmp_r[i] = qureg.stateVec[qq + i].real;
                    tmp_i[i] = qureg.stateVec[qq + i].imag;
                }
                in[0] = _mm512_load_pd(tmp_r);
                in[1] = _mm512_load_pd(tmp_i);
                _mm512_store_pd(tmp_r, _mm512_mul_pd(in[0], contracted_rzz[0]) - _mm512_mul_pd(in[1], contracted_rzz[1]));
                _mm512_store_pd(tmp_i, _mm512_mul_pd(in[0], contracted_rzz[1]) + _mm512_mul_pd(in[1], contracted_rzz[0]));
                for (int i = 0; i < SIMD; i++) {
                    qureg.stateVec[qq + i].real = tmp_r[i];
                    qureg.stateVec[qq + i].imag = tmp_i[i];
                }
