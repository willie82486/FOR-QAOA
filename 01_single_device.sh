...
echo "Running HyQuas"
cd ~/FOR-QAOA-GPU/HyQuas/third-party/cutt
make clean && make -j$(nproc)
cd ~/FOR-QAOA-GPU/HyQuas/ && mkdir -p evaluator-preprocess/parameter-files
cd ~/FOR-QAOA-GPU/HyQuas/benchmark && ./preprocess.sh
cd ~/FOR-QAOA-GPU/HyQuas/scripts && source ./init.sh -DBACKEND=mix -DSHOW_SUMMARY=on -DSHOW_SCHEDULE=on -DMICRO_BENCH=on -DUSE_DOUBLE=on -DDISABLE_ASSERT=off -DENABLE_OVERLAP=on -DMEASURE_STAGE=off -DEVALUATOR_PREPROCESS=on -DUSE_MPI=off -DMAT=7
for Q in {26..27};
do
